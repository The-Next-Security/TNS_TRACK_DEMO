CREATE DEFINER=`root`@`%` FUNCTION `teltonika`.`calcular_temp_interpolada`(
    p_registro_id INT,
    p_channel_id INT,
...
BEGIN
    ...
    SELECT external_temperature, external_temperature_timestamp
    INTO v_temp_anterior, v_timestamp_anterior
    FROM sensor_readings_ubibot
    WHERE channel_id = p_channel_id
    AND external_temperature_timestamp < p_timestamp
    AND external_temperature_timestamp >= DATE_SUB(p_timestamp, INTERVAL 2 HOUR)  -- Ventana de 2 horas máximo
    AND external_temperature IS NOT NULL
    ORDER BY external_temperature_timestamp DESC
    LIMIT 1;
    ...
    WHERE channel_id = p_channel_id
    AND external_temperature_timestamp > p_timestamp
    AND external_temperature_timestamp <= DATE_ADD(p_timestamp, INTERVAL 2 HOUR)  -- Ventana de 2 horas máximo
    AND external_temperature IS NOT NULL
    ...
    IF v_temp_anterior IS NULL AND v_temp_posterior IS NULL THEN
        ...
        SELECT external_temperature INTO v_temp_interpolada
        FROM sensor_readings_ubibot
        WHERE channel_id = p_channel_id
        AND DATE(external_temperature_timestamp) = DATE(p_timestamp)
        AND external_temperature IS NOT NULL
        ORDER BY ABS(TIMESTAMPDIFF(SECOND, external_temperature_timestamp, p_timestamp)) ASC
        LIMIT 1;
