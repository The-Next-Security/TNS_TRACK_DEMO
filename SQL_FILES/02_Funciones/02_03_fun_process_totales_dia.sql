...
BEGIN
    ...
    IF v_registro_existe THEN
        SELECT id_totales_dia INTO v_id_totales_dia
        FROM sem_totales_dia
        WHERE fecha_local = p_fecha_local
        LIMIT 1;
    END IF;
    ...
    IF v_registro_existe THEN
        ...
        FROM sem_totales_dia
        WHERE id_totales_dia = v_id_totales_dia;
    END IF;
    ...
    SELECT
        m.shelly_id,
        p_fecha_local,
        -- Cálculo de energía activa en kWh: (SUM(potencia_activa) * intervalo_segundos) / (3600 * 1000)
    ...
    IF ROW_COUNT() > 0 THEN
        INSERT INTO log_stored_procedures (
            ...
            ) FROM sem_totales_dia WHERE id_totales_dia = COALESCE(v_id_totales_dia, LAST_INSERT_ID())),
            'Actualización exitosa de totales diarios'
        );
        SET v_success = TRUE;
    END IF;
