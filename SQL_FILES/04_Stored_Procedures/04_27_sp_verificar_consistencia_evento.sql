...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_verificar_consistencia_evento`()
    COMMENT 'Verifica la consistencia de datos procesados por el evento'
BEGIN
    -- Verificar fechas en contador_ciclos
    SELECT
        'ANÁLISIS DE FECHAS' AS seccion,
        DATE(fecha_ciclos) AS fecha,
        COUNT(*) AS cantidad_registros,
        COUNT(DISTINCT channel_id) AS channels_unicos,
        CASE
    ...
    FROM (
        ...
        GROUP BY DATE(fecha_ciclos), channel_id
        HAVING COUNT(*) > 1
    ) AS duplicados
    ...
    SELECT
        ...
        COUNT(DISTINCT channel_id) AS channels_distintos,
